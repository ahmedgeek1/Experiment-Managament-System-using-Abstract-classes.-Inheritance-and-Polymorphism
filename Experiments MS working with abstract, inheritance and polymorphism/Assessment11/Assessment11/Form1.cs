...
namespace Assessment11
{
    public partial class Form1 : Form
    {
        ...
        //Experiment exp;
        Space_Experiment exp;
        //Experiment exp1 = new Experiment(); //after adding abstract, using this line to instanciate the class will create error
        ...
        string[,] ExperimentDetArray = new string[51, 9];
        ...
        private void btnCreateExperiment_Click(object sender, EventArgs e)
        {
            ...
            {//if the number of experiments is over 50, then don't allow the user to add more
                ...
                {//if the student name only is knon and the radion button is the one selected
                    if (txtBoxStudentNameKnown.Text != "") //data validation, student name should not be empty
                    {
                        ...
                        {   //if student name is valid, not numeric, then 
                            ...
                            exp = new Space_Experiment(txtBoxStudentNameKnown.Text); //create experiment using the construct passing the student name entered as a parameter
                    ...
                    {   //data validation on the experiment number
                        if (richTextBoxExperimentDescription.Text != "")
                        {
                            if(experimentNumber>0 && experimentNumber <= 50)
                            {
                                ...
                                exp = new Space_Experiment(experimentNumber, txtBoxStudentNameKnown.Text); //create the experiment using the constructor with 2 parameters passed in
                ...
                {
                    MessageBox.Show("Please select the known values options from the 3 radio buttons");
                }
            }
            else
            {
        ...
        {
            int experimentNumber;
            double resultVolume, resultWeight, gravity;
            int volacity;
            ...
            if (int.TryParse(txtBoxExperimentNumberEdit.Text, out experimentNumber))
            {
                if (experimentNumber > 0 && experimentNumber <= 50)
                {
                    if (double.TryParse(txtBoxResultVolumeAdd.Text, out resultVolume) || txtBoxResultVolumeAdd.Text == "")
                    {
                        ...
                        {
                            if (double.TryParse(txtBoxGravity.Text, out gravity) || txtBoxGravity.Text == "")
                            {
                                if (int.TryParse(txtBoxVolacity.Text, out volacity) || txtBoxVolacity.Text == "")
                                {
                                ...
                                {
                                    MessageBox.Show("Please enter a valid velocity value");//else show validation message
                                }

                            }
                            else
                            {
                                MessageBox.Show("Please enter a valid gravity value");//else show validation message
                            }
                                ...
                                if (exp.ResultWeight != 0)
                                {
                                    if (exp.ResultVolume != 0)
                                    {
                                        ...
                                        {//if overwrite then get the listbox index from the table, using the experiment number entered
                                            ...
                                            ExperimentDetArray[vr, 7] = exp.Gravity.ToString();
                                            ExperimentDetArray[vr, 8] = exp.Velocity.ToString();
                                        ...
                                        {   //save the experiment details in the table normally with the next empty index in the array
                                            ...
                                            ExperimentDetArray[indexListBox, 7] = exp.Gravity.ToString();
                                            ExperimentDetArray[indexListBox, 8] = exp.Velocity.ToString();
                                        ...
                                        richTextBoxExperimentDescriptionEdit.Text = "";
                                       ...
                                       // MessageBox.Show(comboBoxColors.SelectedValue.ToString());
                                        ...
                                        {   
                                            ...
                                            ExperimentDetArray[indexListBox, 7] = "";
                                            ExperimentDetArray[indexListBox, 8] = "";
        ...
        private void listBoxExperiments_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {   //if an item is selected in the listbox, then show the form and pass the details of that experiment selected according the the index and the experiment number
                ...
                reviewForm.reviewExperienceDetails(
                 ...
                 ExperimentDetArray[listBoxExperiments.SelectedIndex, 7],
                 ExperimentDetArray[listBoxExperiments.SelectedIndex, 8]);
        ...
        private void groupBoxExperimentDetailsEdit_Enter(object sender, EventArgs e)
        {

        }

        private void txtBoxGravity_TextChanged(object sender, EventArgs e)
        {
        ...
        {
            //we can use the exp object, it will work fine only after the user create an experiment, otherwise it will send back an error because no experiment has been created!
            /*Space_Experiment exp1 = new Space_Experiment();
            exp1.OpenThePodBayDoors();
            exp1 = null;*/
        }
